...
7. **Prose trigger** — "Enable prose trigger? [y/N]" — when enabled, Xavier installs a managed instruction block into `~/.claude/CLAUDE.md` (Claude Code) and a global skill at `~/.cursor/skills/prose-trigger/SKILL.md` (Cursor) that teach the LLM to detect a vocative trigger word (e.g. "Xavier, grill me on this plan") and route through the Xavier router. Default: `no`. Store as `prose-trigger` in config (`yes` or `no`).
