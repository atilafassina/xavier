...
- New `cohorts-index` requires key in the router vocabulary (and `validate-xavier-frontmatter.sh`'s `VALID_REQUIRES`) — lists cohorts under `knowledge/cohorts/`, surfaces each cohort's mission + lesson count, and grants the read-sanction for lesson-records used in ZPD placement and the due-scan.
- `cohort` and `lesson` note types in the Zettelkasten schema (`references/formats/zettelkasten.md`), with a Cohorts / Lessons field reference (cohort: `cohort`, `mission`; lesson: `cohort`, `zpd`, `demonstrated`, `misconceptions`, `sources`, `fluency`, `last_reviewed`).
- Untrusted-content handling throughout `teach`: research/web material, cohort missions, lesson-records, and restored checkpoints are consumed as **nonce-delimited fenced data** (a per-instance token verified absent from the payload, so the content cannot forge its own closing delimiter) and never as instructions (CWE-74); resumed checkpoint slugs are re-validated against the basename allowlist with filename-match and path-containment checks before any filesystem use.
